...
export function ResultScreen({ data, onBack, onStartOver }: ResultScreenProps) {
  ...
  const generatePrompt = () => {
    const featuresText = data.features.map(f => {
...
**Automação:** ${data.messageAutomation === 'whatsapp' ? 'WhatsApp' : 
  data.messageAutomation === 'email' ? 'E-mail' : 
  data.messageAutomation === 'telegram' ? 'Telegram' : 
  data.messageAutomation === 'all' ? 'WhatsApp, E-mail e Telegram' : 
  'Não necessário'}
